...
# The version every first-party package is built with, and what the ISO build
# names the image after (ADR 0008). Read where build-iso.sh reads it.
VERSION := $(shell sed -n 's/^pkgver=//p' aports/alpymist/APKBUILD)
